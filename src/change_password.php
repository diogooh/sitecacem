...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($new_password !== $confirm_password) {
        header("Location: perfil_atleta.php?error=" . urlencode("As senhas não coincidem"));
        exit();
    }

    // Verificar se a nova senha tem pelo menos 8 caracteres
    if (strlen($new_password) < 8) {
        header("Location: perfil_atleta.php?error=" . urlencode("A nova senha deve ter pelo menos 8 caracteres"));
        exit();
    }
    ...
    if (!$user) {
        header("Location: perfil_atleta.php?error=" . urlencode("Usuário não encontrado"));
        exit();
    }

    // Verificar se a senha atual está correta
    if (!password_verify($current_password, $user['password_hash'])) {
        header("Location: perfil_atleta.php?error=" . urlencode("Senha atual incorreta"));
        exit();
    }
    ...
    if (!$stmt->execute()) {
        header("Location: perfil_atleta.php?error=" . urlencode("Erro ao atualizar a senha: " . $stmt->error));
        exit();
    }

    $stmt->close();
    header("Location: perfil_atleta.php?success=password_updated");
    exit();
}
